      ...
      <main className="flex-1 p-5 space-y-4 overflow-y-auto no-scrollbar">
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <ChevronRight className={brandSelected ? "text-[#0B4488]" : "text-slate-300"} />
          </div>
          {brandSelected && (
          ...
          >
            ...
            <div ref={scrollContainerRef} className="flex-1 overflow-y-auto p-6 no-scrollbar">
              <div className="relative mb-6">
                ...
                <input
                  placeholder="Busca..."
                  className="w-full bg-white rounded-[14px] py-5 pl-16 pr-14 font-semibold text-slate-800 outline-none border border-[#B7B7B7] focus:ring-1 focus:ring-[#0B4488]"
                ...
                {filteredResults.map(({ item, layer }) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        {item.logo && (
                          <div className="w-12 h-12 bg-white rounded-sm p-2 flex items-center justify-center border border-slate-50 overflow-hidden">
                            <Image src={getImageUrl(item.logo)} alt={item.name} width={32} height={32} className="object-contain" unoptimized />
                          </div>
                        )}
                        <div className="flex flex-col">
                          <span className={`font-semibold text-[16px] ${isSelected ? "text-[#0B4488]" : "text-slate-800"}`}>{item.name}</span>
