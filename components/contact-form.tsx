...
export default function ContactForm() {
  ...
  // Helper para mapear el tamaño de la imagen a los strings que espera tu API
  ...
  const handleSubmit = useCallback(async () => {
    ...
    // Avisar al UnifiedFooter
    window.dispatchEvent(new CustomEvent("form-submitting", { detail: true }));

    try {
      let finalPersonalImageId = selection.imageCustomUrl;

      // 1. PROCESAMIENTO DE IMAGEN PERSONALIZADA
      if (selection.imageSourceType === "custom" && selection.imageCustomUrl) {
        try {
          const res = await fetch(selection.imageCustomUrl);
          const blob = await res.blob();
          const extension = blob.type.split("/")[1] || "png";

          const file = new File(
            [blob],
            `custom-order-${Date.now()}.${extension}`,
            { type: blob.type }
          );
          ...
          if (!uploadedFile?.id)
            throw new Error("Error al obtener ID de imagen");
          finalPersonalImageId = uploadedFile.id;

          if (selection.imageCustomUrl.startsWith("blob:")) {
            URL.revokeObjectURL(selection.imageCustomUrl);
          }
        } catch (uploadErr) {
          console.error("Error en la subida:", uploadErr);
      ...
      const hasImage =
        selection.imageSourceType !== null ||
      ...
      if (hasImage) {
        orderResponse = await createOrderImage({
          ...
          catalog_image_combo_content:
            selection.imageSourceType === "brand" ? selection.catalogId : null,
          catalog_image:
            selection.imageSourceType === "brand"
              ? selection.catalog_image
              : null,
          image_source_type:
            selection.imageSourceType === "brand" ? "catalog" : "personal",
          personal_image:
            selection.imageSourceType === "custom"
              ? finalPersonalImageId
              : null,
          image_size:
            selection.imageSourceType === "brand"
              ? mapSize(selection.imageBrandConfig.size)
              : mapSize(selection.imageCustomConfig.size),
          image_orientation_degrees:
            selection.imageSourceType === "brand"
      ...
      // 3. GUARDADO DE RESPUESTA EN EL STORE
      ...
      window.dispatchEvent(
        new CustomEvent("form-submitting", { detail: false })
      );
    }
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("trigger-contact-submit", handleTrigger);
  }, [handleSubmit]);
    ...
    return (
      <div className="flex flex-col h-full items-center justify-center p-10 bg-white">
        ...
        >
          ...
          <h2 className="text-3xl font-black text-slate-900 italic tracking-tighter uppercase mb-2 text-center">
            Enviando Orden
          </h2>
      ...
      <main className="flex-1 overflow-y-auto px-4 pt-10 no-scrollbar">
        ...
        <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
          <div className="space-y-2">
            <Label className="text-[14px] font-normal text-slate-700 ml-1">
              Nombre Completo *
            </Label>
            <Input
              id="name"
              ...
              className="h-16 rounded-2xl border-slate-400 font-semibold text-slate-900 bg-slate-50/50 px-6 focus:ring-2 focus:ring-purple-100 transition-all"
            />
          </div>
          ...
          <div className="space-y-1">
            ...
            <Input
              ...
              onChange={handleChange}
              className="h-16 rounded-[14px] border-slate-400 font-semibold text-slate-900 bg-slate-50/50 px-6 focus:ring-2 focus:ring-purple-100 transition-all"
            />
          </div>
          ...
          <AnimatePresence>
            {error && (
              <motion.div
                ...
                className="p-5 bg-red-50 border border-red-100 rounded-2xl flex items-center gap-4 text-red-600 text-[10px] font-black tracking-widest uppercase"
              >
