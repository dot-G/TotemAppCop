...
// Configuración de layout (Inmutable)
const CARD_W = 240;
const CARD_GAP = 16;
...
// Helpers matemáticos fuera del componente para evitar re-declaraciones
...
export function Onboarding({ initialSlides = [] }: OnboardingProps) {
  ...
  // Usamos las props directamente. Eliminamos el hook useOnboarding.
  ...
  // Agrupamos refs de gestos para limpieza
  ...
  // Memoizamos el realIndex para evitar cálculos en cada frame del drag
  ...
  // --- Handlers de Gestos (Lógica original preservada) ---
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    ...
    if (dt > 0) gesture.velocity = (e.clientX - gesture.lastX) / dt;
    gesture.lastX = e.clientX;
  ...
  const onPointerUp = () => {
    ...
    const velocityBias = clamp(-gesture.velocity * 0.4, -0.5, 0.5);
    const snapped = Math.round((dragDelta / STEP) + velocityBias);
    setDragDelta(0);
    moveBySteps(snapped);
  };
  ...
  return (
    ...
    >
      <header className="relative z-10 flex justify-center pt-10 pb-4 shrink-0">
        <img src="/logo-telcel.svg" alt="Telcel Logo" width={100} height={40} className="h-auto w-auto" />
      </header>
      ...
      <div className="flex justify-center gap-2 py-2 shrink-0">
        {slides.map((_, index) => (
          <button
            ...
            className={`h-2 rounded-full transition-all duration-300 ${index === realIndex ? "w-8 bg-[#71E5FF]" : "w-2 bg-white/40"}`}
          />
        ))}
      </div>
