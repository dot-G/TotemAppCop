...
export function StepHeader({ 
  ...
  return (
    <div className="bg-white py-2.5 px-3 flex items-center gap-3 shrink-0 border-b rounded-[14px] z-[50] relative">
      
  {/* Botón Atrás */}
  <button 
    onClick={() => setStep(backTo)} 
    className="w-8 h-8 rounded-full border-2 border-[#0D51A1] flex items-center justify-center text-[#0D51A1] active:scale-90 transition-all shrink-0"
  >
    ...
    <div className="flex flex-1 items-center gap-2.5 min-w-0">
      ...
      <div className="relative w-10 h-10 shrink-0">
        <svg className="w-full h-full -rotate-90" viewBox="0 0 32 32">
          <circle cx="16" cy="16" r="14" fill="none" stroke="currentColor" className="text-slate-100" strokeWidth="3" />
          <motion.circle 
            cx="16" cy="16" r="14"
            fill="none"
      ...
      <div className="flex-1 overflow-hidden relative h-10">
        <AnimatePresence mode="wait">
          ...
          >
            <h2 className="text-[16px] font-semibold text-black leading-tight truncate tracking-tight">
              {title}
            </h2>
